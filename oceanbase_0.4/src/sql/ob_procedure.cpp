...
{
	int ret=OB_SUCCESS;
	for (int64_t i = 0;i < params_.count();i++)//check declare variable exists in procedure parameter list
	{
		ObString* param_name=get_param(i)->param_name_;
		if(param_name->compare(var)==0)
		{
			TBSYS_LOG(USER_ERROR, "declare variable can't same with procedure parameter");
			ret=OB_ENTRY_EXIST;
			break;
		}
	}
	for (int64_t i = 0;i < declare_variable_.count();i++)//判断变量是否重复定义了
	{
		...
		{
			TBSYS_LOG(USER_ERROR, "declare variable have same name");
			ret=OB_ENTRY_EXIST;
			break;
		}
	}
	if(ret==OB_SUCCESS)
	{
		...
		TBSYS_LOG(INFO, "add declare variable %.*s declare_variable_ size=%ld",var.length(),var.ptr(),declare_variable_.count());
	}
	return ret;
}
...
int ObProcedure::open()
{
	...
	TBSYS_LOG(INFO, "procedure Operator parameter size: %ld declare variable size = %ld",get_param_num(),get_declare_var_num());
	int ret = OB_SUCCESS;
	if (OB_UNLIKELY(ObMultiChildrenPhyOperator::get_child_num() <= 0))
	{
	...
	else
	{
		...
		for (int32_t i = 0; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
		{
		  ...
		  else
		  {
			  ...
			  TBSYS_LOG(INFO, "open procedure Operator parameter size: %ld declare variable size = %ld",get_param_num(),get_declare_var_num());
		  }
		}
