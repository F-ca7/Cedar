...
//        int schedule();
        int schedule(common::ConIdxStage which_stage =
            common::ConIdxStage::STAGE_INIT);
//        int start_round();
        int start_round(common::ConIdxStage which_stage =
            common::ConIdxStage::STAGE_INIT);
        //mod e
        int finish_round();    // @TODO realize finish_round()
        void destroy();
        ...
        void construct_tablet_item(const uint64_t table_id,
            ...
            const common::ObRowkey &end_rowkey, common::ObNewRange &range,
      ...
      private:
        ...
        int create_index_handlers(ObIndexHandler** handler, const int64_t size);
        // mod e

        int destroy_index_builders(ObIndexHandler** handler,
            const int64_t size);
        ...
        int get_index_handler(const int64_t thread_no,
            ObIndexHandler* &handler);
