...
{
	TBSYS_LOG(INFO, "zz:ObProcedureCase::open");
	int ret = OB_SUCCESS;
	if (OB_UNLIKELY(ObMultiChildrenPhyOperator::get_child_num() <= 0))
	{
	    ret = OB_NOT_INIT;
	...
	else
	{
		...
		TBSYS_LOG(INFO, "zz:ObProcedureCase num=%d",child_num_);
		for (int32_t i = 0; i < ObMultiChildrenPhyOperator::get_child_num(); i++)
		{
		  ...
		  else if ((ret = op->open()) != OB_INVALID_ERROR)//when 执行成功就不需要执行else了
		  {
			need_do_else=false;
			break;
		  }
		  ...
		  {
			  TBSYS_LOG(INFO, "zz:ObProcedureCase i=%d item is false! go on!",i);
		  }
		}
		if(is_have_else()&&need_do_else&&(ret=else_op_->open())!=OB_SUCCESS)
		{
			TBSYS_LOG(ERROR, "zz:ObProcedureCase need else but else_op open faild");
		}
	}
