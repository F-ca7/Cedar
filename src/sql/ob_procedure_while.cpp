...
int ObProcedureWhile::open()
{
	...
	else
	{
		...
		while(true)
		{
			...
			{
				TBSYS_LOG(ERROR, "zz:ObProcedureWhile expr compute failed");
				break;
			}
			else if(result->is_true())
			{
				TBSYS_LOG(INFO, "zz:ObProcedureWhile expr value is true");

				TBSYS_LOG(INFO, "zz:ObProcedureWhile child_num=%d",ObMultiChildrenPhyOperator::get_child_num());
				/*依次打开while then*/
				for (int32_t i = 0; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
				{
				  ...
				  {
					  TBSYS_LOG(INFO, "Open the %dth child of ObProcedureWhile success", i);
				  }
				}
				if(is_error)
				{
			...
			{
				TBSYS_LOG(INFO, "zz:ObProcedureWhile expr value is false break");
				break;
			}
		}
	}
