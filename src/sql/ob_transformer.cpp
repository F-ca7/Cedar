			...
			if (ret == OB_SUCCESS)
			{
				...
				if (session_info->plan_exists(stmt->get_proc_name(), &stmt_id) == false)
				{
					...
					if(stmt->get_proc_stmt_id()!=common::OB_INVALID_ID)
					{
						...
						{
							TBSYS_LOG(INFO,"rebuild procedure operator success!");
						}
					}
					else
					{
                ...
                ObProcedure *proc_op, SpMultiInsts *mul_inst)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      if_contrl->add_read_var(var_if_expr);
    }
    if (ret == OB_SUCCESS)
    {
      ...
      for(int64_t i = 0; ret == OB_SUCCESS && i < stmt->get_then_stmt_size(); i++)
      {
        ...
        if( OB_SUCCESS != (ret = gen_physical_procedure_inst(logical_plan, physical_plan, err_stat, stmt_id, proc_op, if_contrl->get_then_block())) )
        {}
      }
      if(stmt->have_elseif())
      {
        TBSYS_LOG(WARN, "does not support else if now");
        ret = OB_ERROR;
...
//        }
      ...
      if(stmt->have_else())
      {
        ...
        for(int64_t i = 0; i < else_stmt->get_else_stmt_size(); ++i)
        {
          ...
          if( OB_SUCCESS != (ret = gen_physical_procedure_inst(logical_plan, physical_plan, err_stat, stmt_id, proc_op, if_contrl->get_else_block())) )
          {}
        }
      }
    }
...
//      proc_op->add_inst_d(rw_delta_into_var_inst);
    ...
    {
      SpRwCompInst* rw_comp_inst = proc_op->create_inst<SpRwCompInst>(mul_inst);
  ...
  if (OB_LIKELY(OB_SUCCESS == ret))
  {
    ...
    for (int64_t i = 0; ret == OB_SUCCESS && i < row_num; i++) // for each row
    {
      ...
      for (int64_t j = 0; ret == OB_SUCCESS && j < row_desc_map.count(); j++)
      {
        ...
        // success
        else if (rowkey_info.is_rowkey_column(column_id)) //judge whether it is a rowkey
        {
    ...
    )
{
  ...
  */
  ...
  else
  {
    ...
    //add zt 20151106 : b
    if( rd_base_inst != NULL ) rd_base_inst->set_tid(table_id);
    if( rw_delta_inst != NULL ) rw_delta_inst->set_tid(table_id);
    //add zt 20151106 : e
