...
{
	TBSYS_LOG(INFO, "zz:ObProcedureElse::open()");
	int ret = OB_SUCCESS;
	if (OB_UNLIKELY(ObMultiChildrenPhyOperator::get_child_num() <= 0))
	{
	    ret = OB_NOT_INIT;
	...
	{
		TBSYS_LOG(INFO, "zz:ObProcedureElse child number=%d",ObMultiChildrenPhyOperator::get_child_num());
		for (int32_t i = 0; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
		{
		  ...
		  {
			  TBSYS_LOG(INFO, "Open the %dth child of ObProcedureElse success", i);
		  }
		}
	}
