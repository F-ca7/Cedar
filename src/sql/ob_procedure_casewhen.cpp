...
{
	TBSYS_LOG(INFO, "zz:ObProcedureCaseWhen::open()");
	int ret = OB_SUCCESS;
	if (OB_UNLIKELY(ObMultiChildrenPhyOperator::get_child_num() <= 0))
	{
	    ret = OB_NOT_INIT;
	    TBSYS_LOG(ERROR, "zz:ObProcedureCaseWhen has no child operator, ret=%d", ret);
	}
	else
	{
		//TODO
		...
		{
			TBSYS_LOG(WARN, "zz:ObProcedureCaseWhen case_value expr compute failed");
		}
		else if((ret=case_expr_.calc(tmp_row2,when_value))!=OB_SUCCESS)
		{
			TBSYS_LOG(WARN, "zz:ObProcedureCaseWhen when_value expr compute failed");
		}
		else if((ret=compare_expr_.calc(tmp_row3,compare_value))!=OB_SUCCESS)
		{
			TBSYS_LOG(WARN, "zz:ObProcedureCaseWhen when_value expr compute failed");
		}
		else if(!compare_value->is_true())/*判断是否相等*/
		{
			TBSYS_LOG(INFO, "zz:ObProcedureCaseWhen case expr != when expr");
		...
		else
		{
			...
			for (int32_t i = 0; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
			{
			  ...
			  {
				  TBSYS_LOG(INFO, "zz:ObProcedureCaseWhen child open success!");
			  }
			}
		}
